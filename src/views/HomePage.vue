<template>
  <BaseFrame class="flex justify-center items-start">
    <BaseSurface class="mr-4 flex-initial w-3/5">
      <div class="pb-12">
        <RecipeFeatured :item="featuredRecipe" />
      </div>
    </BaseSurface>
    <BaseSurface class="ml-4 flex-initial w-2/5">
      <div class="pt-4">
        <h2 class="px-5 py-3">Newest Recipes</h2>
        <BaseDivider />
        <PerfectScrollbar
          class="h-[354px]"
          :options="{ wheelSpeed: 0.5, wheelPropagation: false }"
        >
          <RecipeGrid :items="latestRecipes" />
        </PerfectScrollbar>
      </div>
    </BaseSurface>
  </BaseFrame>
...
import { RecipeFeatured, RecipeGrid } from '@/components/recipe';
...
export default {
  ...
  setup: () => {
    const featuredRecipe = computed(() => ({
      ...
      excerpt: `Curabitur euismod scelerisque augue. Cras faucibus at nunc at hendrerit. Vestibulum
    ...
    return { featuredRecipe, latestRecipes };
  },
};
</script>
